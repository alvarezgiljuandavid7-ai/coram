...
function readMetadataRole(user: User): 'admin' | 'premium' | 'member' | null {
  const role = user.app_metadata?.role;
  return role === 'admin' || role === 'premium' || role === 'member' ? role : null;
}
...
export async function getCurrentCoramSession(): Promise<CoramSession> {
  ...
  return {
    ...
    role: metadataRole ?? mappedProfile?.role ?? 'member',
  };
}
