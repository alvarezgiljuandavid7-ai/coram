    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
        {courses.length ? courses.map((course) => (
          ...
          <EmptyContentCard title="Estamos preparando nuevas experiencias de formación" detail="Muy pronto encontrarás clases, mentorías y recursos para fortalecer tu servicio musical." />
        )}
      </div>
      {selected && (
...
function EmptyContentCard({ title, detail }: { title: string; detail: string }) {
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-[oklch(99%_0.004_90)] p-6 text-center shadow-sm lg:col-span-2 2xl:col-span-3">
      <p className="text-sm font-black text-[#0B2545]">{title}</p>
