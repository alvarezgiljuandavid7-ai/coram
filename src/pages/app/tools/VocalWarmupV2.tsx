...
export function VocalWarmupV2() {
  ...
  const notes = scaleForVoice(tools.warmupVoiceClass);
   ...
   return <ExperienceCanvas>
     ...
     <EditorialCard className="p-4 sm:p-5"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#a56b09]">Configuración</p><h2 className="mt-2 font-serif text-[clamp(1.6rem,6vw,2.25rem)] leading-none text-[#0B2545]">Ajusta tu registro</h2><div className="mt-4 grid grid-cols-4 gap-2">{voiceClasses.map((voice) => <button type="button" key={voice} onClick={() => tools.setWarmupVoiceClass(voice)} className={`min-h-10 rounded-xl px-1 text-[11px] font-black transition focus-visible:ring-4 focus-visible:ring-[#2563eb]/20 ${tools.warmupVoiceClass === voice ? 'bg-[#2563eb] text-white shadow-[0_8px_16px_rgba(37,99,235,0.2)]' : 'border border-[#0B2545]/10 bg-white text-[#0B2545]'}`}>{voice[0].toUpperCase() + voice.slice(1)}</button>)}</div><div className="mt-2 grid grid-cols-4 gap-2">{vowels.map((vowel) => <button type="button" key={vowel} onClick={() => tools.setWarmupVowel(vowel)} className={`min-h-10 rounded-xl px-1 text-[11px] font-black transition focus-visible:ring-4 focus-visible:ring-[#2563eb]/20 ${tools.warmupVowel === vowel ? 'bg-[#2563eb] text-white shadow-[0_8px_16px_rgba(37,99,235,0.2)]' : 'border border-[#0B2545]/10 bg-white text-[#0B2545]'}`}>{vowel}</button>)}</div></EditorialCard>
